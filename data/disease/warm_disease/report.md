...
> 生成于 2026-06-12T10:36:50Z；框架：TCM-Disease-Hermes（多智能体检索—审核—抽取—分析）。
> 古今映射为表型对应/候选，非现代诊断；每条结论可回源至原文证据片段。

## 摘要

以现代疾病“温病”为起点，经古病名扩展、表型组合检索、特殊表型补充、形态学精筛与排除性鉴别获取候选条文；再经证据回源、三视角相关性评审与共识裁决完成纳入判断；最终以五层本体映射、关系抽取、复杂网络与时序分析将非结构化古籍文本转化为可计算、可追溯的证据链。共获候选条文 8 条，纳入（Gold+Silver）0 条。
...
- 候选条文：8 条；
- 分级：Gold 0 / Silver 0 / Bronze 4 / Rejected 4；
- 命中分型分布：{'qi': 1, 'shuwen': 1, 'shiwen': 1, 'ying': 1}；
- 核心药物（按中心性）：桔梗、淡豆豉、牛蒡子、甘草、竹葉、荊芥、薄荷、蘆根；
- 涉及朝代：清。

### 2.1 药物网络
核心药物（按 PageRank/度中心性）：桔梗、淡豆豉、牛蒡子、甘草、竹葉。

### 2.2 时序演变
- 清：主要病机 氣分、熱入血分、血分；主要治法 芳香化濁、清營透熱、涼血散血

## 3 讨论

1. 主要发现：“温病”的古籍证治呈现以桔梗、淡豆豉、牛蒡子、甘草、竹葉、荊芥、薄荷、蘆根为核心的药物结构，病机以风、血、湿相关为主线。
