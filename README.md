...
## 疾病知识发现：TCM-Disease-Hermes（以银屑病为例）

可迁移的多智能体疾病框架：现代疾病 → 古籍多层检索（核心召回/表型/特殊型/形态/排除）
→ 候选自治审核（证据回源 + 三视角评审 + 共识分级）→ 五层本体 → 关系抽取 →
药物共现网络（PMI + 度/介数/特征向量/PageRank，纯 Python）→ 朝代时序 → 报告。

```bash
python3 -m hermes disease plan --disease 银屑病      # 分型/古病名/检索式
python3 -m hermes disease run  --disease 银屑病      # 内置示例语料全流程
python3 -m hermes disease run  --disease 银屑病 --use-corpus --no-sample
```

四原则落在代码：召回与精筛分离、正向与排除分离、现代表型与古籍语义分离、
结论证据回源；古今映射只为**候选/表型对应，非诊断**。换一个 `DiseaseProfile`
即可迁移到骨质疏松、类风湿、湿疹等。详见 [docs/DISEASE_HERMES.md](docs/DISEASE_HERMES.md)。
