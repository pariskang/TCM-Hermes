...
## 疾病知识发现：TCM-Disease-Hermes（可迁移，内置 3 病种）
...
python3 -m hermes disease run --disease 类风湿 --use-corpus --no-sample --skills  # 真实金匮语料
python3 -m hermes ask "白疕 鳞屑 血燥"                                # 疾病 Skill 经 Skill RAG 命中
```

内置 `psoriasis`/`osteoporosis`/`rheumatoid_arthritis` 三个 Profile。**类风湿直接命中
...
接入现有 Skill RAG。详见 [docs/DISEASE_HERMES.md](docs/DISEASE_HERMES.md)。

## 接入 Claude Code / Codex / 任意 MCP·CLI Agent

```bash
pip install -e ".[mcp]"
python3 -m hermes mcp          # stdio MCP server，暴露 7 个 Hermes 工具
```

Claude Code（`.mcp.json`）/ Codex（`~/.codex/config.toml`）注册 `python3 -m hermes mcp`
即可在对话中调用古籍检索、Skill 问答、方药溯源、处方匹配、疾病发现等工具；不支持 MCP 的
...
- [docs/DISEASE_HERMES.md](docs/DISEASE_HERMES.md) — 疾病多智能体框架（银屑病/骨质疏松/类风湿）、Disease-Skill、迁移
- [docs/INTEGRATIONS.md](docs/INTEGRATIONS.md) — Claude Code / Codex / MCP·CLI 接入
- [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) — 架构与数据流
- [docs/SAFETY.md](docs/SAFETY.md) — 安全治理边界
- `prompts/` — 五个核心智能体的 v5 提示词；`examples/litellm_multi_model.py` — 多模型示例
- `pytest tests/`：83 项测试覆盖协议不变量（证据子串、门控阈值、修复上限、
  rejected 留档、合并仅用 silver/gold、Skill 输出契约、患者端安全拒绝、
  人审字段全仓扫描）+ litellm 后端/评审小组/绑定校验 + 疾病框架（乾癬→silver、
  圆癣→rejected、药物网络中心性、时序、断点续跑）+ 骨质疏松/类风湿 Profile、Disease-Skill 编译与 Skill RAG 接入、MCP 工具分发。
